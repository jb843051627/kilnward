...
func (a *App) GetCommand(ctx context.Context, id string) (model.OperatorCommand, error) {
	...
	return a.repo.GetCommand(context.Background(), id)
}
...
func (a *App) RunCommand(ctx context.Context, id string) (model.OperatorCommand, error) {
	command, err := a.repo.GetCommand(context.Background(), id)
	if err != nil {
		return command, err
	}
	...
	if err := a.repo.UpdateCommand(context.Background(), command); err != nil {
		return command, err
	}
	if command.LoadID != "" {
