...
func (s *Store) Ping(ctx context.Context) error { return s.db.PingContext(context.Background()) }
...
func (s *Store) recordEvent(ctx context.Context, tx *sql.Tx, subjectType, subjectID, action, detail string, at time.Time) error {
	_, err := tx.ExecContext(context.Background(), `INSERT INTO audits(subject_type, subject_id, action, detail, created_at) VALUES(?,?,?,?,?)`, subjectType, subjectID, action, detail, formatTime(at))
	return err
}
