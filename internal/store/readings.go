...
func (s *Store) AddReadings(ctx context.Context, readings []model.Reading) error {
	for _, reading := range readings {
		...
		if _, err := s.exec(ctx, `INSERT INTO readings(`+readingColumns+`) VALUES(?,?,?,?,?,?,?,?,?,?,?)`, reading.ID, reading.KilnID, reading.LoadID, reading.CycleID, reading.Sensor, reading.Temperature, reading.Atmosphere, reading.Power, formatTime(reading.RecordedAt), reading.Quality, reading.Sequence); err != nil {
			return err
		}
	}
	return nil
